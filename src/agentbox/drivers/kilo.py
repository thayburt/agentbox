...
class KiloDriver:
    ...
    def state_mounts(self, settings: object, host_env: Mapping[str, str]) -> list[MountSpec]:
        ...
        return [
            MountSpec(
                _xdg_path(host_env, "XDG_DATA_HOME", home / ".local" / "share") / "kilo",
                f"{KILO_HOME}/.local/share/kilo",
                "directory",
                create=True,
                chown=True,
    ...
    def init_files(self, settings: object) -> list[InitFileSpec]:
        ...
        return [
            InitFileSpec(
                AGENTBOX_CONFIG_RELATIVE_PATH, read_template("kilo/kilo.jsonc"), "Kilo config"
            )
        ]
    ...
    def config_mounts(
        ...
        mounts = [
            MountSpec(
                _xdg_path(host_env, "XDG_CONFIG_HOME", home / ".config") / "kilo",
                f"{KILO_HOME}/.config/kilo",
                "directory",
                optional=True,
                readonly=True,
                relabel="none",
                description="Kilo XDG config",
            ),
            MountSpec(
                home / ".kilo",
                f"{KILO_HOME}/.kilo",
                ...
                readonly=True,
                relabel="none",
                description="Kilo home config",
            ),
            MountSpec(
                home / ".kilocode",
                f"{KILO_HOME}/.kilocode",
                "directory",
                optional=True,
                readonly=True,
                relabel="none",
                description="Kilo legacy home config",
            ),
        ]
        if host_env.get("KILO_CONFIG_DIR"):
            mounts.append(
                MountSpec(
                    Path(host_env["KILO_CONFIG_DIR"]).expanduser(),
                    "/kilo-host/KILO_CONFIG_DIR",
                    "directory",
                    readonly=True,
                    relabel="none",
                    description="KILO_CONFIG_DIR directory",
                )
            )
        if agentbox_config.exists():
            mounts.append(
                MountSpec(
                    agentbox_config,
                    "/agentbox/config/kilo.jsonc",
                    "file",
                    readonly=True,
                    description="Agentbox Kilo config",
                )
            )
        elif host_env.get("KILO_CONFIG"):
            mounts.append(
                MountSpec(
                    Path(host_env["KILO_CONFIG"]).expanduser(),
                    "/kilo-host/KILO_CONFIG",
                    "file",
                    readonly=True,
                    relabel="none",
                    description="KILO_CONFIG file",
                )
            )
        return mounts
    ...
    def diagnostics(
        ...
        diagnostics.append(
            Diagnostic(
                "Agentbox Kilo config",
                str(agentbox_config),
                "ok" if agentbox_config.exists() else "warning",
                None if agentbox_config.exists() else "run agentbox init to create it",
            )
        )
        if host_env.get("KILO_CONFIG_DIR"):
            ...
            if agentbox_config.exists():
                diagnostics.append(
                    Diagnostic(
                        ...
                        str(source),
                        "warning",
                        "ignored because .agentbox/kilo/kilo.jsonc is active",
                    )
                )
            elif not source.is_file():
                diagnostics.append(
                    Diagnostic(
                        "KILO_CONFIG file", str(source), "error", "required file does not exist"
                    )
                )
        return diagnostics
